...
async function updatePins(userId: string, lngLat: any[]) {
  ...
  const newPins = lngLat.map((lngLatItem: any) => {
    ...
    return { pinId: newPinId, lngLat: lngLatItem };
...
export default function Home() {
  ...
  useEffect(() => {
    const checkUserAndWriteData = async () => {
      ...
      if (!userExists) {
        writeUserData(firebase.auth().currentUser?.uid ?? '', firebase.auth().currentUser?.displayName ?? '', firebase.auth().currentUser?.email ?? '');
      }
    };
  ...
  const updateMarkers = (newMarkers: mapboxgl.Marker[]) => {
    const uniqueMarkers = Array.from(new Set(newMarkers));
    setMarkers(uniqueMarkers);
    updatePins(firebase.auth().currentUser?.uid ?? '', uniqueMarkers.map((marker) => marker.getLngLat()));
  };
